...
filename = "/home/hakem/Desktop/sample.json"
    ...
    for host in item["services"]:
        for CVE in host["vulnerability"]:
            ...
            if (CVE['cve_ids']!='NOCVE'):
                li.append([item['floating_ip'],CVE['cve_ids'], CVE['cvss_score'],host['port'],host['protocol'],host['name']])



df = pd.read_sql("select * from nvd", engine)
df.to_csv("/home/hakem/ag/all.csv")
b = [i[1] for i in li]
df = df[df.id.isin(b)]
print 'test'
